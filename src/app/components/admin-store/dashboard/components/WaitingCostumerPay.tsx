...
const WaitingCostumerPay: React.FC<Props> = ({className, loadingPage, chartOrder, orderData}) => {
  // Count From Order API
  const getStatusCount = (orderData: any[]): number => {
    return (
      orderData?.filter((order) => order?.quotation?.length === 0 && order?.receipt_number === null)
        .length ?? 0
    )
  }
